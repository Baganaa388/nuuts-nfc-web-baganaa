// db.js — SQLite init + helper функцууд
...
function createUserWithUid({ name, nickname, profession, uid }) {
  const tx = db.transaction(() => {
    if (uid) {
      db.prepare("UPDATE users SET uid=NULL WHERE uid=?").run(uid);
    }
    const info = db
      .prepare(
        "INSERT INTO users(name, nickname, profession, uid) VALUES (?,?,?,?)"
      )
      .run(name, nickname || null, profession || null, uid || null);
...
function quickRegisterLink({ name, nickname, profession, uid }) {
  const tx = db.transaction(() => {
    ...
    db.prepare(
      "INSERT INTO users(name, nickname, profession, uid) VALUES (?,?,?,?)"
    ).run(name, nickname || null, profession || null, uid);
  });
  tx();
}
